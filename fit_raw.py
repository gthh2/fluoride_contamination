...
class fit_raw: 
    ...
    def fit_1D(self, dic=None, data=None, offset=-120):
        ...
        ppm_range = np.logical_and(ppm_real >= -123, ppm_real <= -117) #prev values -127, -124.5
        ...
        # set bounds for fit
        bounds = ([-123, 0, -np.inf, 0, -np.inf, -np.inf], [-113, 0.09, np.inf, 3*intensity_range.max(), np.inf, np.inf]) #-126.5 and -124.5
        ...
        ax.set_xlim(-117, -123) #-124.5, 126.5
        plt.show()
